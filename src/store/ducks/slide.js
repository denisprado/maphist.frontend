...
export const INITIAL_STATE = Immutable({
  data: JSON.parse(localStorage.getItem('@maphist:project')),
...
export const openSlideModal = (state, { index }) => state.merge({ modalSlideOpen: true, activeIndex: index });
export const closeSlideModal = (state) => state.merge({ modalSlideOpen: false });
export const nextSlideModal = (state) => (state.activeIndex === state.data.files.length - 1
    ? state.merge({ activeIndex: 0 })
    : state.merge({ activeIndex: state.activeIndex + 1 }));
export const prevSlideModal = (state) => (state.activeIndex === 0
